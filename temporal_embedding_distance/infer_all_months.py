...
from self_supervised_change_detection.datasets import MONTHS, T, load_scaler_npz
from self_supervised_change_detection.utils import normalize_scores, robust_zscore
...
def main():
    ...
    for key, sdf in tqdm(grp, desc="prepare+infer", dynamic_ncols=True):
        ...
        for mc, mv in meta.items():
            row[mc] = mv
        row["mode"] = "temporal_embedding_distance"
